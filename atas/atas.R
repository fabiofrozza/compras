...
atas_sicaf_ler <- function(sicaf_arquivos, dados_pregao) {
  ...
  for (i in seq_along(sicaf_arquivos)) {
    ...
    log_barra_progresso(
      sprintf(
        "Verificando arquivo %s === Ata %s",
        basename(sicaf_arquivos[i]),
        ata[i]
      ),
      pb = pb
    )
    ...
    tryCatch(
      {
        ...
        pdf_responsavel <-
          str_extract(
            pdf_linha_unica, "(?=Dados do Responsável Legal)(?s)(.*?)($)"
          )
        ...
        if (is.na(cnpj[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("CNPJ do pdf", sicaf_arquivos[i])
        } else {
        ...
        razao_social_previo <-
          str_trim(
            str_extract(
              pdf_linha_unica, "(?<=Social:)(?s)(.*?)(?=Nome Fantasia)"
            )
          )
        razao_social_previo <-
          str_replace_all(razao_social_previo, "\\\\n|/", "")
        razao_social[i] <- str_to_upper(str_squish(razao_social_previo))
        nome_arquivo[i] <-
          str_replace_all(razao_social[i], "[\\\\/:*?\"<>|]", "")

        if (is.na(razao_social[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("RAZÃO SOCIAL do pdf", sicaf_arquivos[i])
        } else {
        ...
        endereco[i] <-
          str_to_upper(
            str_trim(
              str_extract(
                pdf_linha_unica, "(?<=Endereço:)(?s)(.*?)(?=\\n|\\\\n)"
              )
            )
          )
        if (is.na(endereco[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("ENDEREÇO do pdf", sicaf_arquivos[i])
        }

        cep[i] <- str_extract(pdf_linha_unica, "\\d\\d\\.\\d\\d\\d\\-\\d\\d\\d")
        if (is.na(cep[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("CEP do pdf", sicaf_arquivos[i])
        }

        cidade[i] <-
          str_trim(
            str_extract(pdf_linha_unica, "(?<=Município / UF:)(?s)(.*?)(?=/)")
          )
        if (is.na(cidade[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("CIDADE do pdf", sicaf_arquivos[i])
        }

        estado_previo <-
          str_trim(
            str_extract(
              pdf_linha_unica, "(?<=Município / UF:)(?s)(.*?)(?=\\n|\\\\n)"
            )
          )
        ...
        if (is.na(estado[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("ESTADO do pdf", sicaf_arquivos[i])
        }

        telefone[i] <-
          str_trim(
            str_extract(
              pdf_linha_unica, "(?<=Telefone:)(?s)(.*?)(?=\\n|\\\\n|Telefone)"
            )
          )
        if (is.na(telefone[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("TELEFONE do pdf", sicaf_arquivos[i])
        }

        email_previo <-
          as.list(
            str_extract_all(
              pdf_linha_unica, "(?<=E-mail:)(?s)(.*?)(?=\\n|\\\\n)"
            )
          )
        ...
        email[i] <-
          paste(
            unique(email_previo[[1]][nchar(email_previo[[1]]) != 0]),
            collapse = ", "
          )
        if (is.na(email[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("E-MAIL do pdf", sicaf_arquivos[i])
        }
        ...
        if (is.na(cpf[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("CPF do pdf", sicaf_arquivos[i])
        }

        responsavel[i] <-
          str_trim(
            str_extract(pdf_responsavel, "(?<=Nome:)(?s)(.*?)(?=\\n|\\\\n)")
          )
        if (is.na(responsavel[i])) {
          msg_erro[[length(msg_erro) + 1]] <-
            paste("RESPONSÁVEL do pdf", sicaf_arquivos[i])
        }
      },
      error = function(e) {
        log_erro(
...
atas_verificar_duplicados <- function(dados_atas) {
  ...
  duplicados <-
    dados_atas[
      dados_atas$cnpj %in% dados_atas$cnpj[duplicated(dados_atas$cnpj)],
      c(6, 8:9)
    ]
