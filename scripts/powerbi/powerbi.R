...
power_bi_main <- function() {
  ...
  switch(script_a_executar,
    "planejamento" = {
      power_bi_planejamento_main(
        utils,
        script_a_executar
      )
    },
...
power_bi_salvar <- function(nome_arquivo, dados, abas, vazios) {
  ...
  arquivo_a_salvar <- file.path(
    pasta$superior,
    sprintf("Dados Visão %s.xlsx", nome_arquivo)
  )

  pb <- log_barra_progresso(
    "Salvando arquivo para o Power BI. Aguarde...",
    length(dados) + 1
  )
  ...
  tryCatch(
    {
      ...
      for (i in seq_along(dados)) {
        ...
        log_barra_progresso(
          sprintf(
            "Salvando arquivo para o Power BI. Aba %s incluída", abas[i]
          ),
          pb = pb
        )
      }
...
power_bi_renomear <- function(script_a_executar) {
  obter_arquivos <- function(pasta) {
    ...
    tryCatch(
      {
        arquivos <- list.files(
          path = file.path(pasta_superior, pasta),
          pattern = "^[^~]",
          recursive = TRUE,
          full.names = TRUE
        )
      ...
      error = function(e) {
        log_erro(
          sprintf(
            "Não foi possível obter os arquivos da pasta %s. Encerrando...",
            pasta
          ),
          e,
          finalizar = TRUE
    ...
    if (length(arquivos) == 0) {
      log_erro(
        sprintf("Nenhum arquivo encontrado na pasta %s. Encerrando...", pasta),
        finalizar = TRUE
      )
    } else {
      arquivos
  ...
  ler_arquivos <- function(arquivos, opcao) {
    ...
    lista_arquivos <- lapply(arquivos, function(arquivo) {
      log_barra_progresso(
        sprintf("Lendo %s: %s", opcao, basename(arquivo)),
        pb = pb
      )
      ...
      tryCatch(
          ...
          if (opcao == "relatório de execução") {
            ...
            read_excel(arquivo) %>%
              mutate(
                ...
                Ano =
                  str_split_i(
                    arquivo, "/", length(str_split_1(arquivo, "/")) - 2
                  ),
                Etapa =
                  str_split_i(
                    arquivo, "/", length(str_split_1(arquivo, "/")) - 1
                  ),
                pregao_mapa = str_remove(Edital, "^0+"),
                arquivo_mapa_ajustado = paste0(
                  dirname(arquivo),
                  "/Mapa ", Ano, " - ", Etapa,
                  " - SPA ", str_sub(Processo, 7, 12),
                  "-", str_sub(Processo, 14, 17),
                  " - PE ", gsub("/", "-", pregao_mapa),
                  ".xlsx"
                )
              )
        ...
        error = function(e) {
          log_erro(
            sprintf("Erro ao processar o %s %s. Ignorando...", opcao, arquivo),
            e,
            alerta = TRUE
          )
          NULL
    ...
    if (length(arquivos_dados) == 0) {
      ...
      log_info(
        sprintf(
          "Foram encontrados %d arquivos de %s válidos.",
          length(arquivos_dados), opcao
        ),
        cores = "highlight1"
      )
    }
  ...
  verificar_duplicidade <- function(dados, agrupar_por, verificar_por) {
    dados_duplicados <- dados %>%
      ...
      summarise(
        qtd_arquivos = n_distinct({{ verificar_por }}),
  ...
  conferir_pregoes <- function(mapas_dados, relatorios_dados) {
    ...
    if (nrow(pregoes_sem_mapa) > 0) {
      log_erro(
        "Os seguintes pregões não possuem mapa e serão ignorados:",
      ...
      walk2(alteracoes$antigo, alteracoes$novo, ~ {
        tryCatch(
          ...
          warning = function(w) {
            log_erro(
              sprintf(
                "Problemas ao renomear %s. Passando para o próximo...",
                tipo_arquivo
              ),
              w
            )
  ...
  mapas_duplicados <-
    verificar_duplicidade(mapas_dados, Processo, mapas_arquivos)
  ...
  if (script_a_executar != "licitacao") {
    ...
    relatorios_duplicados <-
      verificar_duplicidade(
        relatorios_dados, pregao_relatorio, arquivo_relatorio
      )
  }
  ...
  renomear_arquivos(
    controle_mapas, mapas_arquivos, arquivo_mapa_ajustado, "MAPA"
  )

  if (script_a_executar != "licitacao") {
    renomear_arquivos(
      controle_pregoes, arquivo_relatorio,
      arquivo_execucao_ajustado, "RELATÓRIO"
    )
  }

  switch(script_a_executar,
    "renomear" = config_finalizar(),
    "licitacao" = mapas_dados,
    list(
      mapas_dados = mapas_dados,
      relatorios_dados = relatorios_dados
    )
...
power_bi_planejamento_main <- function(utils, script_a_executar) {
  ...
  planilha_obter <- function(ano_licitacao) {
    ...
    planilha_original <- lapply(ano_licitacao, function(ano) {
      log_barra_progresso(
        sprintf("Lendo planilha de Controle de Processos - Ano %s", ano),
        pb = pb
      )
      ...
      tipos_colunas <-
        if (ano == 2021) {
          "ccDccc__licc_________c"
        } else {
          "ccDcc_c__licc__________c"
        }
      ...
      planilha_ano <- tryCatch(
        ...
        error = function(e) {
          log_erro(
            sprintf(
              paste0(
                "Erro ao acessar aba %s da Planilha de Controle. ",
                "Verifique a permissão de leitura e se o link %s está correto."
              ),
              paste("Licitação", ano), url_planilha
            ),
            e
          )
          NULL
    ...
    if (length(planilha_original) == 0) NULL else as.list(planilha_original)
  }
  ...
  planilha_analisar <- function(planilha) {
    ...
    if (nrow(inconsistencias_colunas) > 0) {
      msg_erro <-
        sprintf(
          paste0(
            " Processo %s em %s: diferença nas colunas de ",
            "requerentes, pedidos e itens"
          ),
          inconsistencias_colunas$processo, inconsistencias_colunas$ano
        )
      log_erro(
        "Houve problemas na captura dos dados.", unique(msg_erro),
        alerta = TRUE
      )
    }
    ...
    dados_observatorio <- planilha %>%
      ...
      mutate(
        ...
        itens =
          map2(itens_lista, tamanho_maximo, padronizar_colunas, 0)
      ) %>%
      ...
      mutate(
        resultado =
          case_when(
            ...
            grepl("CAPL", situacao, fixed = TRUE) ~
              "Aguardando envio ou em análise",
            .default = "Não enviado"
          ),
  ...
  if (is.null(planilha_original)) {
    log_erro(
      paste0(
        "Não foi possível obter os dados da ",
        "planilha de Controle de Processos. Encerrando..."
      ),
      finalizar = script_a_executar != "todos"
    )
    return(NULL)
...
power_bi_licitacao_main <- function(utils, mapas_dados, script_a_executar) {
  verificar_resultados <- function(mapas_dados) {
    ...
    mapas_resultados <- mapas_dados %>%
      ...
      select(-any_of(c(
        "Setor Solicitante", "Modalidade", "Situação",
  ...
  verificar_duplicados <- function(mapas_resultados) {
    ...
    # Neste caso, se houver mais de um idUnicoSetor,
    # escolhe apenas o com valor diferente de R$ 0,00
    ...
    if (nrow(duplicados) > 0) {
      log_erro(
        paste0(
          "Itens com informações duplicadas. ",
          "Mantendo apenas os com valor homologado."
        ),
...
power_bi_execucao_main <- function(utils, resultado_renomear,
                                   script_a_executar) {
  ...
  processar_relatorios <- function(relatorios_dados, tipo) {
    ...
    for (processo_a_analisar in processos) {
      ...
      if (is.na(npregao) || nchar(trimws(npregao)) == 0) {
        log_barra_progresso(
          sprintf(
            "Pregão inválido no relatório do processo %s, pulando...",
            processo_a_analisar
          ),
          pb = pb
        )
      ...
      for (i in seq_along(divisores)) {
        ...
        fim <-
          ifelse(
            i == length(divisores),
            nrow(dados_orig), divisores[i + 1] - 1
          )
        if (inicio > fim) next
        ...
        start_row <- if (is.null(config$cabecalho)) {
          ...
          cabecalho_row <-
            which(dados_item[, config$coluna_id] == config$cabecalho)
  ...
  log_info(
    "QUANTIDADE DE MAPAS DE LICITAÇÕES", length(unique(mapas_dados$Processo)),
    "-",
    "QUANTIDADE DE PREGOES FINALIZADOS",
    length(unique(relatorios_dados$pregao_relatorio)),
    cores = "highlight1"
  )
...
power_bi_paalteracoes_main <- function(utils, script_a_executar) {
  planilha_obter <- function(ano_inicial_pa) {
    ...
    abas <-
      c(
        "Processos Administrativos", "Troca de Marca",
        "Cancelamento", "Reequilíbrio"
      )

    pb <-
      log_barra_progresso(
        "Lendo planilha de Processos Administrativos", length(abas)
      )

    planilha_original <- lapply(abas, function(aba) {
      log_barra_progresso(
        sprintf("Lendo planilha de Processos Administrativos - Aba %s", aba),
        pb = pb
      )
      ...
      planilha_aba <- tryCatch(
        ...
        error = function(e) {
          log_erro(
            sprintf(
              paste0(
                "Erro ao acessar aba %s de Processos Administrativos. ",
                "Verifique a permissão de leitura e se o link %s está correto."
              ),
              aba, url_planilha
            ),
      ...
      if (nrow(planilha_aba) == 0) {
        stop(
          sprintf(
            "Não foi possível recuperar os dados da aba %s. Encerrando...", aba
          )
        )
      }
    ...
    if (length(planilha_original) != 4) NULL else as.list(planilha_original)
  }
  ...
  power_bi_salvar(
    nome_arquivo = "SAA",
    dados = list(
      planilha_original$`Processos Administrativos`,
