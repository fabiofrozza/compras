...
author: "`r Sys.getenv('COMPRAS_COORDINATION_NAME')`"
output:
  html_document:
    theme:
      version: 5
...
<img class = "logo_company" src = "`r file.path(dirname(pasta$superior), 'public', 'img', 'company.png')`" />
<img class = "logo_department" src = "`r file.path(dirname(pasta$superior), 'public', 'img', 'department.png')`" />
...
cat("#### Ajustes por solicitação da Unidade ou por iniciativa do Departamento\n")
...
if (dados$stats$qtde_ajustes > 0) {
  ...
  if (importacao$consolidado) {
    pt_ajustes$addRowDataGroups("Processo",
      ...
      outlineTotal = list(
        mergeSpace = "dataGroupsAndCellsAs1",
  ...
  pt_ajustes$defineCalculation(
    calculationName = "Qtde. Itens",
    summariseExpression = "sum(Quantidade_0)"
  )
  pt_ajustes$theme <- "standardtable"
  pt_ajustes$sortRowDataGroups(
    levelNumber = 1,
    orderBy = "calculation",
    sortOrder = "desc"
  )
...
pt_orcamentos$defineCalculation(
  calculationName = "Qtde. Unidades",
...
if (total <= 5) {
  ...
  unidades_com_demanda_tabela <- list(
    dados$filtros$unidades_com_demanda[1:divisor],
...
pt_val_manual$defineCalculation(
  calculationName = "Quantidade demandada",
  summariseExpression = "sum(qtd)"
)
pt_val_manual$addColumnCalculationGroups()
pt_val_manual$sortRowDataGroups(
  levelNumber = 3,
  orderBy = "calculation",
  sortOrder = "desc"
)
...
:::: {.logo_e_data}
<img src = "`r file.path(dirname(pasta$superior), 'public', 'img', 'department.png')`" />
