  ...
  main_dados_criar_dataframe <- function(planilha_longa, importacao, info) {
    ...
    if (importacao$pos_processos) {
      dados_importar <- planilha_longa %>%
        select(
          linha_planilha, qtd, processoFmt, setorResponsavelPesquisa,
      ...
      dados_relatorio <- planilha_longa %>%
        select(
          n_item, linha_planilha, item_compartilhado, qtd, processoFmt,
      ...
      dados_relatorio <- planilha_longa %>%
        select(
          linha_planilha, item_compartilhado, qtd, setorResponsavelPesquisa,
  ...
  tryCatch(
    {
      ...
      if (ano == "FALSE" | is.na(ano) | ano == "") {
        ano <- "Não informado"
      }
      if (etapa == "FALSE" | is.na(etapa) | etapa == "") {
        etapa <- "Não informado"
      }
      if (grupo == "FALSE" | is.na(grupo) | grupo == "") {
        grupo <- "Não informado"
      }
      if (nome_grupo == "FALSE" | is.na(nome_grupo) | nome_grupo == "") {
        nome_grupo <- "Não informado"
      }
...
gerar_comparar_detalhamentos <- function(dados) {
  ...
  if (!is.null(comparacao) && nrow(comparacao) > 0) {
    tryCatch(
      {
        arquivo_descricao <- sprintf(
          "Descrição a ajustar - Grupo %s.xlsx",
          grupo
        )
...
relatorio_gerar_temporario <- function(dados) {
  ...
  relatorio_arquivo <- sprintf(
    "Relatório Gerencial %s %s %s",
...
relatorio_salvar_pdf <- function(relatorio_arquivo) {
  ...
  tryCatch(
    {
      chrome_print(
        input = file.path(pasta$relatorios, paste0(relatorio_arquivo, ".html")),
...
main_inicializar <- function(pacotes, script_a_executar) {
  ...
  pasta$arquivos_importar <- file.path(
    pasta$atual,
    Sys.getenv("COMPRAS_IMPORTACAO_ARQUIVOS_A_IMPORTAR")
  )
  pasta$resumo_pedidos <- file.path(
    pasta$atual,
    Sys.getenv("COMPRAS_IMPORTACAO_RESUMO_PEDIDOS")
  )
  pasta$relatorios <- file.path(
    pasta$atual,
    Sys.getenv("COMPRAS_IMPORTACAO_RELATORIOS")
  )
  pasta$criar <- c(
    pasta$relatorios,
...
main_ambiente <- function(script_a_executar) {
  ...
  importacao$consolidado <-
    if (script_a_executar != "relatorio" ||
      ...
      is.na(importacao$processo_para_relatorio)) {
      TRUE
    } else {
      FALSE
    }
  ...
  tryCatch(
    {
      ...
      log_info("Pastas",
        ...
        c(
          sprintf("Valor mínimo           : %s", importacao$valor_minimo),
