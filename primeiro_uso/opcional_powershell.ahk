...
Instalar:
    ...
    arquivoZip := GetFileName("*.zip", "arquivo de instalação do Powershell em formato .zip")
    ...
    arquivoZip := GetLatestPwsh()
    if (arquivoZip = "") {
        MsgBox, 16, Erro, Não foi possível localizar a última versão do PowerShell para download.`n`nVerifique sua conexão e tente novamente.
        ExitApp
    }

    version := 
    RegExMatch(arquivoZip, "[\d.]+", version)
    url := "https://github.com/PowerShell/PowerShell/releases/download/v" . version . "/" . arquivoZip

    TrayTip, Download do PowerShell, Baixando a última versão do PowerShell. Aguarde..., 60, 1
