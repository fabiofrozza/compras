...
cnpj_verificar <- function(dados, arquivo_alerta) {
  ...
  if (
    ...
    if (nrow(sem_cnpj) > 0) {
      mensagem_sem_cnpj <- c(
        "OS SEGUINTES ARQUIVOS NÃO POSSUEM CNPJ.",
        "=======================================",
    ...
    if (nrow(cnpj_errado) > 0) {
      mensagem_cnpj_errado <- c(
        "OS SEGUINTES FORNECEDORES TEM PROBLEMAS NO CNPJ",
        "=======================================",
...
dados_obter <- function(excel_dados, excel_fornecedores) {
  ...
  tryCatch(
    {
      fornecedores <- data.frame(
        excel_fornecedores, cnpj, razao_social,
