mapas_analisar <- function(mapas_dados, mapas_arquivos) {
  processar_mapa <- function(dados, arquivo) {
    log_barra_progresso(
      label = sprintf("Analisando mapa %s", basename(arquivo)), pb = pb
    )
    ...
    colunas_manter <-
      c(
        "processo", "edital", "data_homologacao", "item", "cod_item", "unidade",
    ...
    tryCatch(
      {
        dados <- dados %>%
          ...
          mutate(
            valor =
              ifelse(valor_homologado == 0, valor_referencia, valor_homologado),
            resultado = case_when(
          ...
          select(
            cod_item, unidade, descricao, especificacao, detalhamento,
        ...
        log_erro(
          sprintf(
            paste0(
              ...
              "Abra o arquivo %s e verifique se está de acordo com o Manual. ",
              "Encerrando..."
            ),
            basename(arquivo)
          ),
...
mapas_ler <- function(mapas_arquivos) {
  ...
  ler_um_arquivo <- function(arquivo) {
    tryCatch(
      {
        log_barra_progresso(
          sprintf("Processando: %s", basename(arquivo)),
          pb = pb
        )
      ...
      error = function(e) {
        log_erro(
          sprintf(
            paste0(
              "Falha ao ler %s. ",
              "Abra o arquivo e verifique se está conforme o Manual. ",
              "Continuando com o próximo..."
            ),
            arquivo
          ),
          e
...
mapas_obter <- function() {
  ...
  tryCatch(
    {
      # get .xls files
      mapas_arquivos <- list.files(
        path = file.path(pasta$mapas),
        pattern = "^[^~]*.xls*",
        full.names = TRUE,
        recursive = TRUE
      )
    ...
    error = function(e) {
      log_erro(
        paste0(
          ...
          "Verifique a pasta MAPAS. Encerrando..."
        ),
        e,
        finalizar = TRUE
      )
...
mapas_salvar <- function(mapas, script_a_executar) {
  salvar_planilha <- function(dados, nome_arquivo) {
    ...
    tryCatch(
      ...
      error = function(e) {
        log_erro(
          sprintf("Não foi possível salvar o arquivo %s", nome_arquivo),
          e
        )
        FALSE
      },
...
mapas_main <- function() {
  ...
  script_a_executar <-
    utils_verificar_script(
      scripts_permitidos = c("processo", "grupo"),
      script_padrao = "grupo"
    )
