catmat_obter_tr <- function(lista_itens_tr) {
  ...
  tryCatch(
    ...
    error = function(e) {
      log_erro(
        sprintf(
          "Não foi possível ler a lista de itens do TR %s. Encerrando...",
          lista_itens_tr
        ),
        e,
        finalizar = TRUE
  ...
  processo <-
    str_extract_all(
      as.character(tr), "23080\\.\\d{6}/\\d{4}-\\d{2}",
      simplify = TRUE
    )
  ...
  if (length(linha_cabecalho) == 0 || is.na(linha_cabecalho)) {
    log_erro(
      paste0(
        "Linha inicial das colunas 'Grupo/Item' e ",
        "'Descrição' não encontrada. Encerrando..."
      ),
      finalizar = TRUE
    )
  }
...
catmat_obter_margens <- function() {
  ...
  tryCatch(
    {
      planilha_margens <-
        read_excel(
          file.path(pasta$fontes, "Margens.xlsx"),
    ...
    error = function(e) {
      log_erro(
        paste0(
          "Não foi possível ler a planilha de margens de preferência ",
          "(_fontes/Margens.xlsx). Encerrando..."
        ),
        e,
        finalizar = TRUE
      )
  ...
  colunas_exigidas <-
    c(
      "NCM", "Descrição", "Regra de origem", "Margem normal",
      "Regra de qualificação", "Margem adicional"
    )
  ...
  if (!all(colunas_exigidas == colunas_planilha)) {
    log_erro(
      paste0(
        "Há diferença no nome das colunas do arquivo Margens.xlsx. ",
        "Elas devem ser exatamente iguais a:"
      ),
      colunas_exigidas,
      finalizar = TRUE
    )
  ...
  # +1 porque o Excel começa na linha 1 (cabeçalho)
  tryCatch(
    {
      planilha_margens$linha_planilha <-
        1:nrow(planilha_margens) + 1
    },
    error = function(e) {
      log_erro("Erro ao adicionar coluna com números das linhas do Excel.",
        e,
        finalizar = TRUE
      )
    }
  ...
  if (any(!margens$NCMvalido)) {
    ...
    log_erro(
      paste0(
        "Os seguintes NCMs não correspondem à descrição. ",
        "Verifique a planilha de margens de preferência nas seguintes linhas:"
      ),
      ncm_invalidos,
      finalizar = TRUE
    )
...
catmat_consultar_api <- function(codigo_item) {
  ...
  resposta <- tryCatch(
    ...
    error = function(e) {
      log_erro(
        "Não foi possível acessar a API. Verifique sua conexão com a internet.",
        alerta = TRUE
      )
      e
    }
  ...
  if (status_code(resposta) == 200) {
    ...
    if (!is.null(dados$resultado) && length(dados$resultado) > 0) {
      return(data.frame(
        ...
        "Status do item" =
          ifelse(
            is.null(dados$resultado$statusItem),
            NA,
            ifelse(
              dados$resultado$statusItem == TRUE,
              "Localizado", "Não localizado"
            )
          ),
        "NCM" =
          ifelse(
            is.null(dados$resultado$codigo_ncm),
            NA, dados$resultado$codigo_ncm
          ),
        "Aplica margem preferência" =
          ifelse(
            is.null(dados$resultado$aplica_margem_preferencia),
            NA, dados$resultado$aplica_margem_preferencia
          ),
        "Resultado consulta" = "Sucesso",
        "descricaoItem" =
          ifelse(
            is.null(dados$resultado$descricaoItem),
            NA, dados$resultado$descricaoItem
          ),
        "codigoGrupo" =
          ifelse(
            is.null(dados$resultado$codigoGrupo),
            NA, dados$resultado$codigoGrupo
          ),
        "nomeGrupo" =
          ifelse(
            is.null(dados$resultado$nomeGrupo),
            NA, dados$resultado$nomeGrupo
          ),
        "codigoClasse" =
          ifelse(
            is.null(dados$resultado$codigoClasse),
            NA, dados$resultado$codigoClasse
          ),
        "nomeClasse" =
          ifelse(
            is.null(dados$resultado$nomeClasse),
            NA, dados$resultado$nomeClasse
          ),
        "codigoPdm" =
          ifelse(
            is.null(dados$resultado$codigoPdm),
            NA, dados$resultado$codigoPdm
          ),
        "nomePdm" =
          ifelse(
            is.null(dados$resultado$nomePdm),
            NA, dados$resultado$nomePdm
          ),
        "codigoItem" =
          ifelse(
            is.null(dados$resultado$codigoItem),
            NA, dados$resultado$codigoItem
          ),
        "itemSustentavel" =
          ifelse(
            is.null(dados$resultado$itemSustentavel),
            NA, dados$resultado$itemSustentavel
          ),
        "descricao_ncm" =
          ifelse(
            is.null(dados$resultado$descricao_ncm),
            NA, dados$resultado$descricao_ncm
          ),
        "dataHoraAtualizacao" =
          ifelse(
            is.null(dados$resultado$dataHoraAtualizacao),
            NA, dados$resultado$dataHoraAtualizacao
          ),
        stringsAsFactors = FALSE
      ))
    } else {
...
catmat_consultar_lista <- function(origem = "local") {
  # Opção não utilizada, pois a lista de CATMATs é muito extensa
  # e é melhor mantê-la localmente
  if (origem == "google") {
    gs4_deauth()
  ...
  tryCatch(
    ...
    error = function(e) {
      log_erro(
        paste0(
          "Não foi possível ler a lista de CATMATs ",
          "(_fontes/Lista CATMAT.xlsx). Encerrando..."
        ),
        e,
        finalizar = TRUE
      )
  ...
  colunas_exigidas <-
    c(
      "Código do Grupo", "Nome do Grupo", "Código da Classe", "Nome da Classe",
      "Código do PDM", "Nome do PDM",
      "Código do Item", "Descrição do Item", "Código NCM"
    )
  ...
  if (all(colunas_exigidas == colunas_planilha)) {
    ...
    log_erro(
      paste0(
        "Há diferença no nome das colunas do arquivo Lista CATMAT.xlsx. ",
        "Elas devem começar na linha 3 e ser exatamente iguais a:"
      ),
      colunas_exigidas,
      finalizar = TRUE
    )
...
catmat_resultados_lista <- function(catmat_tr) {
  ...
  resultados_lista <- catmat_tr %>%
    ...
    mutate(
      "Status item" =
        ifelse(
          is.na(`Código do Grupo`), "Não localizado", "Localizado"
        )
    ) %>%
    rename(`NCM` = "Código NCM") %>%
    select(
      "Item", "CATMAT", "Status item", "NCM",
      "Código do Grupo", "Nome do Grupo",
      "Código da Classe", "Nome da Classe",
      "Código do PDM", "Nome do PDM", "Descrição do Item"
    )
  ...
  tryCatch(
    {
      ...
      saveWorkbook(
        wb,
        file.path(pasta$arquivos_gerados, arquivo_saida),
        overwrite = TRUE
      )
    },
    error = function(e) {
...
catmat_main <- function() {
  ...
  pasta$tr <- file.path(pasta$atual, Sys.getenv("COMPRAS_CATMAT_TR"))
  pasta$arquivos_gerados <- file.path(
    pasta$atual,
    Sys.getenv("COMPRAS_CATMAT_ARQUIVOS_GERADOS")
  )
  ...
  if (is.na(lista_itens_tr)) {
    log_erro(
      "Nenhum arquivo com lista de itens do TR encontrado. Encerrando...",
      finalizar = TRUE
    )
  }
  ...
  script_a_executar <-
    utils_verificar_script(
      scripts_permitidos = c("api", "lista"),
      script_padrao = "api"
    )
